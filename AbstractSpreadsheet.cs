...
namespace SS
{
    ...
    public abstract class AbstractSpreadsheet
    {
        ...
        {
            //state that current name has been visited
            visited.Add(name);

            //iterates through each string that is a dependent of the current one
            foreach (String n in GetDirectDependents(name))
            {
                //if a circular dependency occure
                if (n.Equals(start))
                {
                    throw new CircularException();
                }
                //if the item hasnt already been visted, recursively call Visit function until all cells have been visited
                //or cells have no more direct dependents
